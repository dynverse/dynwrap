...
read_file("https://raw.githubusercontent.com/dynverse/comp1/master/R_feather/definition.yml") %>%
  paste0("```yaml\n", ., "\n```") %>% 
  cat()
```
...
```{r, echo=FALSE}
comp1_url <- function(name, x) glue::glue("[{name}](https://github.com/dynverse/comp1/tree/master/{x})")
comp1_urls <- function(names, xs) glue::collapse(map2_chr(names, xs, comp1_url), ", ")
input_url <- function(name, x) glue::glue("[{name}](https://github.com/dynverse/comp1/tree/master/inst/example_inputs/{x})")
inputs_url <- function(names, xs) glue::collapse(map2_chr(names, xs, input_url), ", ")

tribble(
  ~output_format, ~description, ~example_script, ~example_input,
...
tribble(
  ~output_format, ~description, ~examples,
  "text", "csv and json files", comp1_urls(c("python", "R"), c("python_text", "R_text")),
  "feather", "[Feather](https://github.com/wesm/feather) dataframe files", comp1_urls(c("python", "R"), c("python_feather", "R_feather")),
  "rds", "R binary files", comp1_urls(c("R"), c("R_rds")),
...
```{r, echo=FALSE, results="asis"}
read_file("https://raw.githubusercontent.com/dynverse/comp1/master/R_feather/Dockerfile") %>%
  paste0("```Dockerfile\n", ., "\n```") %>% 
  cat()
```
...
```{r, echo=FALSE, results="asis"}
read_file("https://raw.githubusercontent.com/dynverse/comp1/master/R_feather/run.R") %>%
  paste0("```r\n", ., "\n```") %>% 
  cat()
```
