...
#' @param input Names of input data, such as expression, counts or some [priors]
#' @param input_optional Names of optional input data, such as expression, counts, or some [priors]
...
create_docker_ti_method <- function(
  ...
  if (!is.null(definition)) {
    ...
    if(is.null(input_optional)) input_optional <- definition$input_optional
  ...
  testthat::expect_true(is.character(input_optional))
  ...
  input_ids <- input
  input_ids_optional <- input_optional
  param_ids <- names(par_set$pars)
  output_ids <- output

  if (!any(input_ids %in% input_processors$id)) {
    stop("Invalid input: ", setdiff(input_ids, input_processors$id))
  }

  if (!any(input_ids %in% input_processors$id)) {
    stop("Invalid optional input: ", setdiff(input_ids_optional, input_processors$id))
  }

  if (!any(output_ids %in% output_processors$id)) {
    stop("Invalid output: ", setdiff(input_ids, output_processors$id))
  }
  ...
  run_fun <- function() {
    ...
    for (input_id in input_ids) {
      save_input(get(input_id, environment()), input_id, dir_input)
    }

    for (input_id in input_ids_optional) {
      input <- get(input_id, environment())
      if(!is.null(input)) {
        save_input(input, input_id, dir_input)
      }
    }
  ...
  environment(run_fun) <- list2env(lst(input_ids, input_ids_optional, param_ids, output_ids))
